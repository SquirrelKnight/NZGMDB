...
if __name__ == "__main__":
    ...
    df = pd.read_csv(
        '/Volumes/SeaJade 2 Backup/NZ/NZ_EQ_Catalog/converted_output/earthquake_source_table_relocated.csv',
        low_memory=False,
    )
...
#         )  
    ...
    df['NGASUB_TectClass_HikPuy'] = df.parallel_apply(
        lambda x: ngasub2020_tectclass_v2(
            x['lat'],
            x['lon'],
            x['depth'],
            R_a = HP_a,
    ...
    df.to_csv(
        out_path + '/earthquake_source_table_relocated_tectclass.csv',
        mode='w',
        index=False,
        )
