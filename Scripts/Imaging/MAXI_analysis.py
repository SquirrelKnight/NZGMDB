...
event_df = pd.read_csv(directory+'inputs/orig_events/earthquake_source_table_with_cmt.csv',low_memory=False)
# event_df['depth'] = event_df['z']
event_df['evid'] = event_df['evid'].astype('str')

revised_df = pd.concat([pd.read_csv(f,low_memory=False) for f in glob.glob(directory+'output/catalog_test/*_origins.csv')])
